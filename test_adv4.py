...
def main():
    ...
    for ttin in range(4):
        ...
        wm=torch.load('wm_'+str(layer_sel)+'.pt',map_location=lambda storage, loc: storage)
        
        fp=open('labels_'+str(layer_sel)+'.json') # labels for wm i.e., the labels of the test set.
        label=json.load(fp)
        fp.close()
        ...
        activity_layer[layer_sel]=np.array(temp_vec)
        ...
        for yin in activity_layer:
            ...
            if xin==yin:
                ...
                for zin in range(len(pred_n)):
                    temp=temp+np.dot(pred[zin],post[zin])
                layer_corr[(xin, yin)]=temp/float(len(pred_n))
                values.append(temp/float(len(pred_n)))
