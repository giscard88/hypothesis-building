...
class CogMem_torch:
    '''
    In principle, it is the same as noname.py noname.py expects to receive all inputs in a single batch. This version is to relax the      expectation and mini-batches should work with this version  
    '''
    ...
    def Mat_batch(self, roV, label_,flag=False):
        ...
        if flag:
            ...
            sel_vecs=roV[idx_t,:]
            self.wm=self.wm.view(self.inD,-1)
...
class CogMem_label_torch:
    ...
    def Mat_batch(self, roV, label_,flag=False):
        ...
        if flag:
            ...
            idx_t=torch.from_numpy(idx)
            sel_vecs=roV[idx_t,:]
            sel_vecs=sel_vecs.view(self.inD,-1)
