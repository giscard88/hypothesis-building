...
#                        help='threshold values used when cogmemry is generated')
    ...
    threshold_0=[0.65, 0.75] 
    threshold_1=[0.65, 0.75] 
    threshold_2=[0.65, 0.75]
    threshold_3=[0.65, 0.75]
    range_v={0.65:0.014,0.75:0.007}
    ...
    if os.path.exists("confusion_sel"): # comprehensive confusion
        pass
    else:
    ...
    for th in thresholds_:
        ...
        #pylab.figure(ttin+1)
        pylab.imshow(corr,cmap='jet', vmax=range_v[th])
        pylab.colorbar() 
        pylab.savefig('confusion_sel/L'+str(layer_sel_)+'_'+str(th)+'.png')
        pylab.savefig('confusion_sel/L'+str(layer_sel_)+'_'+str(th)+'.eps')
        pylab.close()    
