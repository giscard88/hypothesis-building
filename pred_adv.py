...
def test(args, model, device,hookF,adv_data,adv_target):
    model.eval()
    test_loss = 0
    correct = 0
    ...
    with torch.no_grad():
        ...
        adv_data, adv_target = adv_data.to(device), adv_target.to(device)
        output = model(adv_data)
        test_loss += F.nll_loss(output, adv_target, reduction='sum').item() # sum up batch loss
        pred = output.argmax(dim=1, keepdim=True) # get the index of the max log-probability
        correct += pred.eq(adv_target.view_as(pred)).sum().item()
    ...
    test_loss /= len(adv_data)

    print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
        test_loss, correct, len(adv_data),
        100. * correct / len(adv_data)))
...
def main():
    ...
    adv_data=torch.load('adv_input.pt')

    print (adv_data)
    adv_target=torch.load('adv_label.pt') 
    ...
    pred_n=test(args, model, device, hookF,adv_data,adv_target)
